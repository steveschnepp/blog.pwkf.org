...
tags: munin tutorial development
...
Note that the [guidelines on contributing](https://github.com/munin-monitoring/munin/blob/master/CONTRIBUTING.md) back are specified directly in the git repo.
...
First, let's assume you have a working user cgi configuration (ie `~user/cgi/whatever` is working). If not you should refer yourself to the local documentation of your preferred webserver. Note that nginx will _not_ work, as [it does not support CGI](http://wiki.nginx.org/ThttpdCGI).
